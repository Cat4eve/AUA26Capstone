...
This version stores both amplitude and phase of the detector field in separate PNG files,
in addition to the intensity hologram PNG. Decoding prefers the complex amplitude+phase
representation when available, because it preserves more wave information than intensity alone.

Even with amplitude+phase saved, this remains a research prototype rather than a production
LLM compressor. Use the resulting metrics as experimental evidence rather than guaranteed recovery.
"""
...
def gabor_inline_field(
    ...
    Returns:
      total_field: complex detector field = reference + object
      object_field: complex object contribution alone
    ...
    ref = np.full_like(obj, complex(reference_amplitude, 0.0))
    total_field = ref + obj
    intensity = np.abs(total_field) ** 2
    return total_field, obj, intensity.astype(np.float64)


def save_real_png(path: Path, data: np.ndarray) -> Tuple[float, float]:
    dmin = float(np.min(data))
    dmax = float(np.max(data))
    if abs(dmax - dmin) < EPS:
        scaled = np.zeros_like(data, dtype=np.uint8)
    else:
        scaled = np.round(255.0 * np.clip((data - dmin) / (dmax - dmin), 0.0, 1.0)).astype(np.uint8)
    Image.fromarray(scaled, mode="L").save(path)
    return dmin, dmax


def load_real_png(path: Path, dmin: float, dmax: float) -> np.ndarray:
    gray = np.asarray(Image.open(path).convert("L"), dtype=np.float64) / 255.0
    return gray * (dmax - dmin) + dmin


def save_phase_png(path: Path, phase: np.ndarray) -> Tuple[float, float]:
    pmin = -math.pi
    pmax = math.pi
    wrapped = np.angle(np.exp(1j * phase))
    scaled = np.round(255.0 * ((wrapped - pmin) / (pmax - pmin))).astype(np.uint8)
    Image.fromarray(scaled, mode="L").save(path)
    return pmin, pmax


def load_phase_png(path: Path, pmin: float = -math.pi, pmax: float = math.pi) -> np.ndarray:
    gray = np.asarray(Image.open(path).convert("L"), dtype=np.float64) / 255.0
    return gray * (pmax - pmin) + pmin


def save_complex_field_pngs(amp_path: Path, phase_path: Path, field: np.ndarray) -> Dict[str, float]:
    amplitude = np.abs(field)
    phase = np.angle(field)
    amin, amax = save_real_png(amp_path, amplitude)
    pmin, pmax = save_phase_png(phase_path, phase)
    return {
        "amplitude_min": amin,
        "amplitude_max": amax,
        "phase_min": pmin,
...
def load_complex_field_from_pngs(amp_path: Path, phase_path: Path, amplitude_min: float, amplitude_max: float, phase_min: float = -math.pi, phase_max: float = math.pi) -> np.ndarray:
    amplitude = load_real_png(amp_path, amplitude_min, amplitude_max)
    phase = load_phase_png(phase_path, phase_min, phase_max)
...
def decode_radial_displacements(
    ...
    signal may be either:
      - a complex field (preferred; amplitude+phase reconstruction), or
      - a real intensity-derived proxy (fallback).
    """
    signal = np.asarray(signal)
    if np.iscomplexobj(signal):
        signal = signal.astype(np.complex128) - np.mean(signal.astype(np.complex128))
    else:
        signal = signal.astype(np.float64) - np.mean(signal.astype(np.float64))
...
def write_decoded_processing_vtps(
    out: Path,
    encoded_indices: np.ndarray,
    decoded_values: np.ndarray,
    signed_est: np.ndarray,
    radial_scores: np.ndarray,
    base_points: np.ndarray,
    decoded_points: np.ndarray,
    side: int,
) -> None:
    """Save VTPs that visualize the decoding result and the reconstructed hemisphere."""
    rows = encoded_indices // side
    cols = encoded_indices % side
    decoded_plane_points = np.column_stack([
        rows.astype(np.float64),
        cols.astype(np.float64),
        decoded_values.astype(np.float64) + 1.0,
    ])
    common_point_data = {
        "decoded_value": decoded_values.astype(np.float64),
        "decoded_signed_value": signed_est.astype(np.float64),
        "radial_score": radial_scores.astype(np.float64),
        "linear_index": encoded_indices.astype(np.int32),
    }
    write_vtp_points(
        out / 'decoded_plane_points_value_plus_1.vtp',
        decoded_plane_points,
        point_data=common_point_data,
    )
    write_vtp_points(
        out / 'decoded_hemisphere_base_points.vtp',
        base_points,
        point_data=common_point_data,
    )
    write_vtp_points(
        out / 'decoded_hemisphere_points_radial.vtp',
        decoded_points,
        point_data=common_point_data,
    )
...
def command_encode(args: argparse.Namespace) -> None:
    ...
    total_field, object_field, intensity = gabor_inline_field(
        points=points,
        holo_size=holo_size,
    ...
    # Save the classical intensity hologram for inspection and compatibility.
    hmin, hmax = save_real_png(out / "gabor_hologram.png", intensity)
    save_real_png(out / "gabor_hologram_intensity.png", intensity)

    # Save complex detector field as amplitude + phase PNGs.
    complex_meta = save_complex_field_pngs(
        out / "gabor_field_amplitude.png",
        out / "gabor_field_phase.png",
        total_field,
    )

    if args.save_float_hologram:
        np.save(out / "gabor_hologram_float.npy", intensity)
        np.save(out / "gabor_field_complex.npy", total_field)
        np.save(out / "gabor_field_object.npy", object_field)
    ...
    meta = {
        ...
        "field_amplitude_png_min": complex_meta["amplitude_min"],
        "field_amplitude_png_max": complex_meta["amplitude_max"],
        "field_phase_png_min": complex_meta["phase_min"],
        "field_phase_png_max": complex_meta["phase_max"],
        ...
        "notes": [
            ...
            "Complex detector field is saved as separate amplitude and phase PNG files.",
            "Decode step also saves reconstructed plane and hemisphere VTP files for inspection.",
            "Complex-field decoding is preferred when amplitude and phase files are present; intensity-only decode remains available as fallback.",
        ],
    }
    ...
    print(f"[encode] hologram intensity: {holo_size} x {holo_size} -> {out / 'gabor_hologram.png'}")
    print(f"[encode] complex field PNGs: {out / 'gabor_field_amplitude.png'} and {out / 'gabor_field_phase.png'}")
...
def command_decode(args: argparse.Namespace) -> None:
    ...
    decode_mode = getattr(args, "decode_mode", "auto")
    if decode_mode not in {"auto", "complex", "intensity"}:
        raise ValueError(f"Unsupported decode mode: {decode_mode}")

    complex_npy_path = out / "gabor_field_complex.npy"
    amp_png_path = out / "gabor_field_amplitude.png"
    phase_png_path = out / "gabor_field_phase.png"

    signal = None
    used_decode_mode = None

    if decode_mode in {"auto", "complex"}:
        if bool(args.use_float_hologram) and complex_npy_path.exists():
            total_field = np.load(complex_npy_path)
            signal = total_field - complex(float(meta["reference_amplitude"]), 0.0)
            used_decode_mode = "complex-float"
        elif amp_png_path.exists() and phase_png_path.exists():
            total_field = load_complex_field_from_pngs(
                amp_png_path,
                phase_png_path,
                float(meta["field_amplitude_png_min"]),
                float(meta["field_amplitude_png_max"]),
                float(meta.get("field_phase_png_min", -math.pi)),
                float(meta.get("field_phase_png_max", math.pi)),
            )
            signal = total_field - complex(float(meta["reference_amplitude"]), 0.0)
            used_decode_mode = "complex-png"
        elif decode_mode == "complex":
            raise FileNotFoundError("Complex decode requested, but amplitude/phase outputs were not found.")

    if signal is None:
        if bool(args.use_float_hologram) and (out / "gabor_hologram_float.npy").exists():
            intensity = np.load(out / "gabor_hologram_float.npy")
        else:
            intensity = load_real_png(out / "gabor_hologram.png", float(meta["hologram_png_min"]), float(meta["hologram_png_max"]))
        signal = intensity.astype(np.float64) - float(meta["reference_amplitude"]) ** 2
        used_decode_mode = "intensity"

    base_points = hemisphere_base_points_from_indices(encoded_indices, side, radius)
    used_search_steps = int(args.radial_search_steps) if int(args.radial_search_steps) > 0 else int(meta.get("radial_search_steps", 33))
    signed_est, radial_scores = decode_radial_displacements(
        ...
        search_steps=used_search_steps,
    ...
    decoded_points = displace_points_radially(base_points, signed_est, radius, float(meta["value_height_scale"]))
    ...
    if not getattr(args, 'no_vtp', False):
        write_decoded_processing_vtps(
            out=out,
            encoded_indices=encoded_indices,
            decoded_values=decoded_values,
            signed_est=signed_est,
    ...
    print(f"[decode] radial search steps: {used_search_steps}")
    print(f"[decode] mode: {used_decode_mode}")
    if not getattr(args, 'no_vtp', False):
        print(f"[decode] decoded VTPs: {out / 'decoded_plane_points_value_plus_1.vtp'}, {out / 'decoded_hemisphere_base_points.vtp'}, {out / 'decoded_hemisphere_points_radial.vtp'}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_dec.add_argument("--decode-mode", choices=["auto", "complex", "intensity"], default="auto", help="Prefer complex amplitude+phase decoding, or force intensity fallback")
    p_dec.add_argument("--no-vtp", action="store_true", help="Skip decoded processing VTP outputs")
    ...
    p_all.add_argument("--decode-mode", choices=["auto", "complex", "intensity"], default="auto", help="Prefer complex amplitude+phase decoding, or force intensity fallback")
